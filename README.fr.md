...
[![Hugging Face Space](https://img.shields.io/badge/%F0%9F%A4%97%20Space-en%20ligne-blue)](https://huggingface.co/spaces/JonathanFernandez/pret_a_depenser)

# Prêt à Dépenser, API de Scoring Crédit

Déploiement en production d'un modèle de scoring crédit (LightGBM, 20 features) sous forme d'API sur Hugging Face Spaces : pipeline CI/CD en deux étapes, 94 % de couverture de tests pytest, monitoring de la dérive des données (Evidently) sur trafic de production simulé.

**Démo en ligne** : https://huggingface.co/spaces/JonathanFernandez/pret_a_depenser
...
├── scripts/
│   ├── extract_demo_examples.py      # Génère data/demo_examples.json
...
│   └── demo_examples.json            # Exemples préchargés de la démo (versionné)
├── docs/
│   ├── notes.md
│   └── screenshots/
...
L'interface Gradio s'ouvre dans le navigateur, avec trois exemples préchargés (profil accepté, refusé, à la limite du seuil) pour tester l'API dès le premier clic. L'API accepte les 20 features suivantes (toutes optionnelles, les valeurs manquantes sont imputées par la médiane via le pipeline sklearn) :
...
Les 3 exemples affichés dans l'interface sont extraits du dataset d'entraînement, pas inventés, pour que les probabilités affichées soient réalistes :

```bash
uv run python -m scripts.extract_demo_examples
```

Écrit `data/demo_examples.json` (versionné, contrairement au reste de `data/`), lu automatiquement par `src/app.py` au démarrage.
...
![Pipeline CI/CD réussi](https://raw.githubusercontent.com/Jojo4911/credit-scoring-mlops/main/docs/screenshots/pipeline_success.png)
...
![Space Hugging Face en cours d'exécution](https://raw.githubusercontent.com/Jojo4911/credit-scoring-mlops/main/docs/screenshots/hf_space_running.png)
